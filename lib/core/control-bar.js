...
export class ControlBar {
   ...
   *   inputs:[{key,label,value?,placeholder?,width?,options?}],  // options → dropdown
  ...
  constructor(mount, cfg, on = {}) {
    ...
    if (hasBuildRow) {
      ...
      inputs.forEach((inp) => {
        ...
        // `options` → a fixed CHOICE (dropdown); otherwise a typed parameter (text box).
        // Either way it is a `u-named` element with a .value, so values()/presets/
        // restoreDefaults treat the two identically.
        const el = document.createElement(inp.options ? "select" : "input");
        el.className = "u-named"; el.dataset.key = inp.key;
        if (inp.options) {
          el.innerHTML = inp.options.map((o) => {
            const v = o.value ?? o;
            return `<option value="${v}">${o.label ?? v}</option>`;
          }).join("");
          el.value = inp.value ?? (inp.options[0].value ?? inp.options[0]);
        } else {
  ...
  restoreDefaults() {
    ...
    this.$named.forEach((el) => {
      ...
      // a dropdown with no spec default falls back to its first option — "" would
      // leave the select with nothing selected
      el.value = (inp && inp.value) ?? (el.tagName === "SELECT" ? el.options[0]?.value ?? "" : "");
    });
