...
export class DualDemo {
  constructor(mount, spec) {
    ...
    spec.panels.forEach((pn) => {
      ...
      const rend = pn.renderer(cv);
      attachPanZoom(cv, rend); // no-op for non-scaling renderers
      this.titles.push(title); this.renderers.push(rend);
    });
  ...
  _stat(idx, snap, frame) {
    ...
    if (pn.stat) return pn.stat(snap, frame);
  ...
  _seq() {
    // ANIMATED lockstep replay of the whole sequence (Build-always-replays):
    // one trace per panel, concatenated key by key — ⏩ sweeps it if it's long
    const op = this.spec.op;
    const per = this.spec.panels.map(() => []);
    (this.spec.sequence || []).forEach((v) =>
      this.structs.forEach((s, k) => per[k].push((this.spec.panels[k].op ?? op)(s, v))));
    this.traces = per.map((ts) => concatTraces(ts));
    this.len = Math.max(...this.traces.map((t) => t.length)); this.i = 0; this.$scrub.max = this.len - 1;
    this._draw(0); this._play();
  }
