...
//                                         // DEFAULT: menu when the bar would hold more than 3
//                                         // actions (Build + ops + Clear), buttons otherwise
...
export class FullDemo {
  ...
  constructor(mount, spec) {
    ...
    // opsUI: explicit spec wins; otherwise a BUSY bar (more than 3 actions,
    // counting Build, the ops, and Clear) defaults to the compact mode-menu +
    // Run, while a short bar keeps the classic buttons
    const modeCount = (hasBuild ? 1 : 0) + (spec.ops || []).length + (chrome.showClear ? 1 : 0);
    const menuUI = chrome.showOps && (spec.opsUI ? spec.opsUI === "menu" : modeCount > 3);
