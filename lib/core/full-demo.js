...
export class FullDemo {
  ...
  constructor(mount, spec) {
    ...
    bar.innerHTML = `
      ...
      <span class="u-sep"></span>`}
      ${needsValue ? `<label>value</label><input class="u-val" value="${spec.valInitial ?? ""}"${spec.valWidth ? ` style="width:${spec.valWidth}px"` : ""} placeholder="${spec.valPlaceholder ?? "key / lo hi"}">` : ""}
      ${namedInputs}
  ...
  _build() {
    ...
    if (this.spec.buildRaw) {
      ...
      const traces = [this.spec.buildRaw(this.inst, this.$seq ? this.$seq.value : "")];
