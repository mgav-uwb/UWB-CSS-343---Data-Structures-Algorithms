...
//                                         // `options` makes it a DROPDOWN instead of a text box —
//                                         // for a fixed choice (the heap's build method) rather
//                                         // than a typed parameter (the B-tree's M)
...
//   buildAll?: (inst, keys, vals) => Trace, // if given, Build (and the initial
...
//                                         // one-shot build (e.g. heapify). `vals`
//                                         // holds the named inputs, so a spec can
//                                         // offer SEVERAL build methods behind one
//                                         // dropdown (the heap: insert vs heapify)
...
export class FullDemo {
  ...
  _build() {
    ...
    if (this.spec.buildAll) {
      ...
      const trace = this.spec.buildAll(this.inst, keys, this._vals());
      this._updateOpEnable();
