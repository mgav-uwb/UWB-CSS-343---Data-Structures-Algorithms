...
// clicking a node to select it, then pressing operation buttons that act on the
// selection. Each op returns a Trace (or {trace, sel} to move the selection).
// For the "rotate any subtree" demo. Requires the renderer to expose nodeAt().
//
// spec = {
...
//   selectMsg?:(key)=>string, hint?, autoSelect?, costs?, chrome?, width?, height?
// }
...
export class InteractiveDemo {
  constructor(mount, spec) {
    ...
    (spec.ops || []).forEach((op) => {
      const b = document.createElement("button"); b.textContent = op.name;
      if (op.ghost) b.className = "ghost"; b.onclick = () => this._run(op); bar.appendChild(b);
    });
    const rst = document.createElement("button"); rst.textContent = "Reset"; rst.className = "ghost";
    rst.onclick = () => this._reset(); bar.appendChild(rst);
