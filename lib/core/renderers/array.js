...
export class ArrayRenderer {
  ...
  draw(snapshot, hl = {}) {
    ...
    // scale cell down if the row is too wide to fit — but never below the
    // configured scale floor: past it, grow the canvas and let the wrapper
    // scroll instead of squeezing cells under the fixed label font
    const minCell = (this.minScale ?? RENDER_LIMITS.minScale) * this.cell;
    sizeCanvas(this, 2 * M + n * minCell + (n - 1) * gap, this.H);
    ctx.clearRect(0, 0, this.W, this.H);
    const cell = Math.max(4, Math.min(this.cell, (this.W - 2 * M - (n - 1) * gap) / n));
    ...
    if (bars) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        ctx.fillStyle = text; ctx.font = "600 11px ui-monospace, Menlo, monospace";
        ctx.textAlign = "center"; ctx.textBaseline = "top";
        ctx.fillText(String(a[i]), x + cell / 2, baseline + 4);

        ctx.fillStyle = COLORS.faint; ctx.font = "500 10px ui-monospace, Menlo, monospace";
        ctx.textAlign = "center"; ctx.textBaseline = "top";
