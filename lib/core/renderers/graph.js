...
export class GraphRenderer {
  ...
  draw(snapshot, hl = {}) {
    ...
    // The LABEL point sits at t = 0.38 — toward the edge's FIRST vertex —
    // so symmetric crossing edges (chords through a circle's center, the
    // K2,2 X pattern) don't stack their weight bubbles on the shared midpoint.
    const LABEL_T = 0.38;
    const curveOf = (e) => {
      ...
      const t = LABEL_T, u = 1 - t; // bezier point (exact lerp when bend = 0)
      return { p, q, cx, cy,
        mx: u * u * p.x + 2 * u * t * cx + t * t * q.x,
        my: u * u * p.y + 2 * u * t * cy + t * t * q.y };
    };
