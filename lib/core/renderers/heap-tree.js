...
// ONE renderer for an array-backed COMPLETE binary tree (heaps stored 0-indexed,
// parent (k-1)>>1, children 2k+1/2k+2 — see structures/heap.js). Draws the SAME
// flat-array snapshot as ArrayRenderer, and reads the SAME index-keyed highlight
// sets (active/compare/pointers/done/danger) — heaps allow duplicate values, so
...
const parentOf = (k) => (k - 1) >> 1;
const leftOf = (k) => 2 * k + 1;
const rightOf = (k) => 2 * k + 2;
...
export class HeapTreeRenderer {
  ...
  draw(snapshot, hl = {}) {
    ...
    const depth = Math.floor(Math.log2(n)); // deepest row index (0-based)
    ...
    }).call(this, 0, this.M, this.W - this.M, 0);
    this._pos = pos;
