...
export class MatrixRenderer {
  ...
  draw(snapshot, hl = {}) {
    ...
    // — but never below the configured scale floor: past it, grow the canvas
    // (grids grow BOTH ways) and let the wrapper / page scroll instead
    const minCell = (this.minScale ?? RENDER_LIMITS.minScale) * this.cell;
    sizeCanvas(this, 2 * M + headerW + minCell * cols, 2 * M + headerH + minCell * rows);
