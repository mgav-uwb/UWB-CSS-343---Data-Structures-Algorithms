...
export class MultiwayTreeRenderer {
  ...
  draw(snapshot, hl = {}) {
    ...
    // ---- pass 3: fit to canvas — uniformly zoom (x AND y, plus box size and
    // font) by whichever of width/height is tighter, so a deep OR a wide tree
    // both stay fully on-canvas and keys stay legible instead of clipping.
    // The zoom is FLOORED at minScale: past the floor the canvas grows and
    // the wrapper scrolls, so keys never shrink into illegibility. ----
    const minScale = this.minScale ?? RENDER_LIMITS.minScale;
    ...
    const fit = Math.max(Math.min(widthScale, heightScale, 1), minScale);
    ...
    if (this._resizedW) { this._resizedW = false; centerScrollOn(this, posMap.get(snapshot).x); }
    ...
    const H = Math.max(10, this.h * fit);
    const fontPx = Math.max(7, Math.round(14 * fit));
    ...
    (function nodes(node) {
      ...
      // key labels — font shrinks with the node so keys stay legible instead
      // of overflowing a compartment that got smaller than the fixed 14px text
