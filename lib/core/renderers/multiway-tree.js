...
// keys it holds). Self-contained: no imports, mirrors core/renderers/tree.js.
...
export class MultiwayTreeRenderer {
  ...
  draw(snapshot, hl = {}) {
    ...
    const availW = Math.max(1, this.W - 2 * this.M);
    const widthScale = totalWidth > availW ? availW / totalWidth : 1;

    const neededH = TOP + maxDepth * vgap + this.h / 2;
    const availH = Math.max(1, this.H - this.M);
    const heightScale = neededH > availH ? availH / neededH : 1;

    const fit = Math.min(widthScale, heightScale, 1);
    const drawnWidth = totalWidth * fit;
    const offsetX = this.M + (availW - drawnWidth) / 2;
