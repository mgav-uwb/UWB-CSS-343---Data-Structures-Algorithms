  ...
  draw(snapshot, hl = {}) {
    ...
    const edges = function (t) {
      ...
      kidsOf(t).forEach((c) => {
        ...
        if (c.edge != null) {
          const f = 0.62;   // toward the CHILD: the midpoint collides with the parent's sub-label
          edgeBadges.push({ x: p.x + (q.x - p.x) * f, y: p.y + (q.y - p.y) * f, lab: c.edge, dim: fadedSet.has(c.key) });
        }
    ...
    edgeBadges.forEach(({ x, y, lab, dim }) => {
      if (dim) ctx.globalAlpha = 0.22;
      ctx.beginPath(); ctx.arc(x, y, 9, 0, 2 * Math.PI); ctx.fillStyle = COLORS.edge; ctx.fill();
