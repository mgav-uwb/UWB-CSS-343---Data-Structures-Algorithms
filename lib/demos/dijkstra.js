...
export const dijkstraDemo = {
  ...
  blurb: "Weighted digraph on vertices 0–7 with an editable edge list (triples \"u v w\"). Repeatedly settle the nearest unsettled vertex and relax its outgoing edges — labels become shortest-path distances and the accent edges form the shortest-paths tree. (This is the array-scan variant: its entire state IS the labels; the O(E log V) version keeps the same tentative distances in a priority queue.) Append \"7 1 -20\" to see a negative weight break the greedy guarantee (flagged in red by the final edge re-check).",
