...
export const fibDemo = {
  id: "fib",
  title: "Fibonacci — naive vs memo vs 2 vars",
  blurb: "The whole DP arc on one function. Naive recursion recomputes the same values exponentially often (the 'times' row counts the waste — 2·fib(n+1)−1 calls). Memoization caches each fib(k) once (2n−1 calls). The two-variable version never holds more than two cells — Θ(1) space. Run all three at the same n and compare the counters.",
  make: () => new DP(),
  initial: "",
  noBuild: true,
  stateMsg: () => "type n (2–16) and pick a version — compare the call counters",
...
export const lcsDemo = {
  ...
  blurb: "Fill the (|a|+1)×(|b|+1) length table: a diagonal +1 on a character match, otherwise the max of the cell above and the cell to the left. Traceback from the bottom-right corner recovers the subsequence itself. Type any two words.",
  make: () => new DP(),
  initial: "",
  noBuild: true,
...
export const editDistanceDemo = {
  ...
  ops: [
    { name: "Run edit distance", run: (s, _v, vals) => s.editDistance(vals.a.toLowerCase().replace(/[^a-z]/g, "") || "kitten", vals.b.toLowerCase().replace(/[^a-z]/g, "") || "sitting") },
  ],
};
