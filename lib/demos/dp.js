...
export const knapsackDemo = {
  ...
  about: `
      ...
      <h3>Change W and watch the answer change</h3>
      <p>The item set is fixed — A(w2/v3) B(w3/v4) C(w4/v5) D(w5/v6) — so the capacity is the only
      variable, and it is the one the greedy counterexample is about:</p>
      <ul>
  ...
  costs: ["compare", "read", "write"],
  chrome: { showValue: false },
  inputs: [{ key: "w", label: "W (1–14)", value: "5", width: 60 }],
  presets: [
    { name: "W = 5 (the lecture example)", values: { w: "5" } },
    { name: "W = 8", values: { w: "8" } },
    { name: "W = 12 (A drops out!)", values: { w: "12" } },
    { name: "W = 14 (everything fits)", values: { w: "14" } },
  ],
  height: 280,
  ops: [
...
export const editDistanceDemo = {
  ...
  about: `
      ...
      <p>The green traceback is the <em>edit script</em>, not just the count: each step's direction
      names an operation, which is what a spell-checker or a diff actually reports. Weighting the
      three operations differently (a substitution matrix) changes the constants and nothing else —
      that generalization is Needleman–Wunsch alignment.</p>`,
  links: [
