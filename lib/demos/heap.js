...
const note = (s, msg) => {
  const t = new Tracer();
  t.step(msg, { snapshot: s.snapshot() });
  return t.trace();
};
 ...
 *  made, so their counters are directly comparable. */
const buildAll = (s, keys, vals, method) => {
  const name = method || "insert";
  const known = name === "insert" || name === "heapify";
  const parts = [];
  if (!known) {
    parts.push(note(s, `unknown build method ${name.toUpperCase()}( … ) — building by INSERT; this demo knows INSERT and HEAPIFY`));
  }
  if (!keys.length) {
    parts.push(note(s, "nothing to build — try INSERT(1..20:RAND) or HEAPIFY(1..20:RAND)"));
    return concatTraces(parts);
  }
  parts.push(known && name === "heapify"
