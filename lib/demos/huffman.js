...
export const huffmanDemo = {
  ...
  blurb: "A greedy, bottom-up optimal prefix code: repeatedly merge the two least-frequent trees in a forest until one remains. More frequent symbols end up shallower → shorter codes. The numbers in the initial box are frequency COUNTS for a, b, c, … (not percentages — the classic CLRS sample just happens to sum to 100); try equal ones for a balanced (no-win) tree. Encode walks the trie root-to-leaf per character and decodes the bits back.",
  make: () => new Huffman(),
