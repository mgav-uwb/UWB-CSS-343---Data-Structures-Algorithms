...
export const kruskalDemo = {
  ...
  blurb: "Weighted undirected graph on vertices 0–7 with an editable edge list (triples \"u v w\"). Sort all edges by weight, then add each one unless its endpoints already share a union-find root — the second row shows that forest; same root means the edge would close a cycle, so it's skipped — stopping after V-1 accepted edges. Variant: pull edges from a MIN-HEAP instead of fully sorting — with the early stop you often never even look at the heavy tail.",
  costs: ["compare"],
