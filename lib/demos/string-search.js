...
const clean = (v, dflt) => {
  const s = expandRepeats(String(v ?? "")).toUpperCase().replace(/[^A-Z]/g, "").slice(0, 24);
  return s || dflt;
};
...
export const stringSearchDemo = {
  ...
  blurb: "Brute force retries the whole pattern at every shift, sliding by one on a mismatch. KMP precomputes a failure table so the text pointer i never backs up — same first match, fewer comparisons on repetitive text. Run both on the same pair and compare the compare counters — the boxes take the repeat shorthand, so the adversarial pair is just text A^11B, pattern A^3B.",
  ...
  costs: ["compare"],
  inputs: [
    { key: "text", label: "text", value: DTEXT, width: 170, placeholder: "…or A^11B" },
