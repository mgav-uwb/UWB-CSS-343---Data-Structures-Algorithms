...
export class AdaptiveHuffman {
  ...
  encodeCodec(text = "abracadabra") {
    ...
    for (let i = 0; i < chars.length; i++) {
      ...
      if (!seen) {
        ...
        this.cx.newSyms++;
        t.step(`after NYT come 8 FIXED bits — '${ch}' in plain ASCII, ${ascii8(ch)} (${ch === SPACE ? 32 : ch.charCodeAt(0)}). The grey bits were only the ROUTE to NYT`,
  ...
  decodeCodec(bits = "") {
    ...
    while (i < stream.length) {
      ...
      let sym;
      if (node.nyt) {
        t.step(`the leaf is NYT — "not yet transmitted", so this is NOT a codeword: the next 8 bits are a literal. Those grey bits were only the ROUTE here`,
