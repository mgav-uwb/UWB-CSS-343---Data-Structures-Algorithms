...
export class AdaptiveHuffman {
  ...
  encodeCodec(text = "abracadabra") {
    ...
    for (let i = 0; i < chars.length; i++) {
      ...
      t.count("compare");
      // ── the first test: does this symbol have a leaf yet? ────────────────
      t.step(seen
        ? `'${ch}': in the tree already → walk from the root to its leaf and send the turns`
        : `'${ch}': NOT in the tree → walk to NYT, then send the symbol in the clear`,
      ...
      if (!seen) {
        t.count("link", 2);
        t.step(`NYT splits: a new NYT and a leaf for '${ch}' — the alphabet just grew`,
