...
export class DP {
  ...
  _rodTree(prices, n) {
    ...
    const build = (len, edge) => {
      const node = { key: `n${id++}`, label: String(len), len };
  ...
  rodCutTreeMemo(prices = [1, 5, 8, 9], n = null) {
    ...
    const bestPath = []; const reusedAt = []; const cutsTaken = []; {
      ...
      const solvedAt = {};
      (function scan(node) {
        if (!skipped.has(node.key) && node.value != null && !(node.len in solvedAt)) solvedAt[node.len] = node.key;
        node.kids.forEach(scan);
      })(root);
      let node = root;
      while (node) {
        bestPath.push(node.key);
        const nxt = node.kids.find((c) => c.value != null && price[c.edge] + c.value === node.value);
        if (!nxt) {
          ...
          if (node.len > 0 && solvedAt[node.len] && solvedAt[node.len] !== node.key) reusedAt.push(solvedAt[node.len]);
          break;
        }
