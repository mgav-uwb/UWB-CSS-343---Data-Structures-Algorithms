...
export class DP {
  ...
  rodCutTree(prices = [1, 5, 8, 9], n = null) {
    ...
    // still has to cut. Bottom-up, deepest first, so a node is only ever labelled
    // after the children it is a max over — and the root's cell ends up being
    // the answer, assembled out of its children's cells plus the edge prices.
    const byDepth = []; (function d(node, k) { (byDepth[k] ||= []).push(node); node.kids.forEach((c) => d(c, k + 1)); })(root, 0);
    for (let k = byDepth.length - 1; k >= 0; k--) {
      const level = byDepth[k];
      level.forEach((node) => {
        node.value = node.kids.length ? Math.max(...node.kids.map((c) => price[c.edge] + c.value)) : 0;
      });
      t.step(k === byDepth.length - 1
        ? `each node is now worth the best obtainable for the length it still has: cut(0) = 0, an empty rod earns nothing`
        : `one level up: a node is the max over its edges of price + the child's value. ${level.map((x) => `cut(${x.len}) = ${x.value}`).filter((v, i, a) => a.indexOf(v) === i).slice(0, 4).join(", ")}${level.length > 4 ? " …" : ""}`,
        { snapshot: snap(), highlight: { cur: level.map((x) => x.key) } });
    }
