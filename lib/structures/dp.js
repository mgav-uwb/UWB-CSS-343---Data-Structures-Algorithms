...
export class DP {
  ...
  _rodTree(prices, n) {
    ...
    const build = (len, edge) => {
      const node = { key: `n${id++}`, label: String(len), len, sub: "" };
      if (edge != null) node.edge = edge;
      node.kids = [];
  ...
  rodCutTree(prices = [1, 5, 8, 9], n = null) {
    ...
    t.step(`all ${cuttings.length} scored — the optimum is ${win.cuts.join(" + ")} = ${best}; the rest score ${others.join(", ")}. Every one of them was a path in this tree`,
      { snapshot: snap(), highlight: { done: win.path } });
    ...
    if (whole && whole !== win) {
      t.step(`selling it WHOLE is just another path — one edge, ${price[n]}, against the optimum's ${best}`,
        { snapshot: snap(), highlight: { done: win.path, danger: [whole.leaf.key] } });
    }
  ...
  rodCutTreeMemo(prices = [1, 5, 8, 9], n = null) {
    ...
    const solve = (node, path) => {
      ...
      memo[node.len] = best; node.sub = `=${best}`;
    ...
    t.step(`done — best = ${best}. Lit: ${live} nodes = 1 + ${N}·${N + 1}/2, the root plus the table's ${(N * (N + 1)) / 2} candidates. Greyed: ${full - live} of the full tree's ${full} — that is what one cache did`,
      { snapshot: snap(), highlight: { faded: fadedNow() } });
    return t.trace();
  }
  ...
  rodCutTreeTab(prices = [1, 5, 8, 9], n = null) {
    ...
    const price = [0, ...(clean.length ? clean : [1, 5, 8, 9])];
    const K = price.length - 1;
    const N = Math.max(1, Math.min(Math.trunc(n) || 4, K, 5));
    const t = new Tracer();
    const best = [0], cut = [0];
    const roots = [];
    const clone = (x) => ({ ...x, kids: (x.kids || []).map(clone) });
    ...
    roots.push({ key: "L0", label: "0", sub: "=0", kids: [] });
    t.step(`bottom-up: fill the lengths in order, smallest first. best[0] = 0, and nothing above it exists yet`,
      { snapshot: snap(), highlight: { done: ["L0"] } });

    for (let len = 1; len <= N; len++) {
      const node = { key: `L${len}`, label: String(len), sub: "", kids: [] };
      let m = -1, ci = 0;
      for (let i = 1; i <= len; i++) {
        const v = price[i] + best[len - i];
        node.kids.push({ key: `L${len}r${i}`, label: String(len - i), sub: `=${best[len - i]}`, edge: i, kids: [], tint: "#1e8e3e" });
        links++; t.count("read").count("compare");
        if (v > m) { m = v; ci = i; }
      }
      roots.push(node);
      t.step(`best[${len}]: read the ${len} cell${len === 1 ? "" : "s"} below it — ${node.kids.map((k, j) => `price[${j + 1}]+best[${k.label}] = ${price[j + 1]}+${best[+k.label]}`).join(", ")}`,
        { snapshot: snap(), highlight: { cur: [node.key] } });
      best[len] = m; cut[len] = ci; node.sub = `=${m}`;
      t.count("write");
      t.step(`best[${len}] = ${m}, first cut ${ci}. No recursion happened, and no length was solved twice`,
    ...
    t.step(`done — best[${N}] = ${best[N]} by ${pieces.join(" + ")}. ${N + 1} cells and ${links} links = ${N}·${N + 1}/2, against the full tree's ${2 ** N} nodes. Same answer, no recursion, nothing explored twice`,
      { snapshot: snap(), highlight: { done: roots.map((r) => r.key) } });
    return t.trace();
  }
