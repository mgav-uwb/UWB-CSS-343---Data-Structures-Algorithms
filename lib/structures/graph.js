...
export class Graph {
   ...
   *  Out-of-range / self-loop / duplicate pairs are dropped; no pairs → sample DAG. */
  build(keys) {
    ...
    const list = pairs.length ? pairs.filter(ok) : EDGES;
    const seen = new Set();
    this.edges = [];
    list.forEach(([u, v]) => {
      const k = `${u}>${v}`;
  ...
  dfs(start = 0) {
    ...
    const self = this;
    (function visit(u) {
      visited.add(u); onStack.add(u); t.count("visit");
      ...
      onStack.delete(u); finished.push(u);
