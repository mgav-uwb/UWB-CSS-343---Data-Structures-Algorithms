...
export class LinearProbing {
  /** @param {number} M @param {{resizeAt?:number}} [opts] resizeAt = load-factor
   *  threshold that triggers doubling (default 0.5); pass Infinity for a
   *  fixed-size table (lecture demos BEFORE resizing is introduced — a full
   *  table then rejects inserts instead of looping). */
  ...
  _insertSilent(k) {
    if (this.n >= this.M) return; // fixed-size table full — refuse
    let i = hash(k, this.M);
  ...
  insert(k) {
    const t = new Tracer();
    if (this.n >= this.M) {
      t.step(`table full (n = M = ${this.M}) — a fixed-size open-addressing table cannot take another key; it needs a RESIZE`,
    ...
    if (this.n / this.M >= this.resizeAt) {
      t.step(`load factor ${this.n}/${this.M} ≥ ${this.resizeAt} — resize to ${this.M * 2} and rehash every key`, { snapshot: this.snapshot(), highlight: {} });
...
// linear-probing demo owns the resize story). remove() is a plain list
// unlink: chaining's delete is trivial, the contrast to open addressing.
export class SeparateChaining {
  ...
  /** insert(k) — hash to the bucket, scan it for a duplicate, append. Never
   *  resizes; a long bucket just gets longer (that is the α story). */
  insert(k) {
    const t = new Tracer();
