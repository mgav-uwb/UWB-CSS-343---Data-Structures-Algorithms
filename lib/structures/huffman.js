...
export class Huffman {
  ...
  encodeCodec(text = "cab") {
    ...
    c.text = [...String(text)].map((ch) => (ch === " " ? SPACE : ch)).map((ch) => ({ t: ch, ci: this._ci(ch) }));
    c.bits = []; c.out = [];
    const known = this.codes();
    ...
    for (let i = 0; i < c.text.length; i++) {
      ...
      t.step(`'${ch}': is it in the tree? yes — walk from the root to its leaf, sending each turn`,
        { snapshot: this._codecFrame({ act: 0, roles: ROLES, cur: [i, null, null], done: [i, null, null], partial: true }),
  ...
  decodeCodec(bits = "") {
    ...
    for (let i = 0; i < c.bits.length; i++) {
      ...
      if (!node.isLeaf) {
        t.step(`bit ${b} → go ${b === "0" ? "left" : "right"} — not a leaf, read another bit (${run} so far)`,
          { snapshot: this._codecFrame({ act: 1, roles: ROLES, cur: [null, i, null], done: [null, i, null] }),
      ...
      c.out.push({ t: sym, ci });
      t.count("write");
      t.step(`bit ${b} → go ${b === "0" ? "left" : "right"} — a leaf, so the codeword ends: ${run} = '${sym}', emitted, back to the root`,
