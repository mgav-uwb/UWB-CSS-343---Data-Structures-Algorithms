...
export class Huffman {
  ...
  encodeCodec(text = "cab") {
    ...
    for (let i = 0; i < c.text.length; i++) {
      ...
      while (!node.isLeaf) {
        ...
        t.step(last
          ? `'${ch}' = ${code} — leaf reached, ${c.bits.length} bits so far`
          : `'${ch}': ${goLeft ? "left → 0" : "right → 1"}   (code so far ${code})`,
  ...
  decodeCodec(bits = "") {
    ...
    for (let i = 0; i < c.bits.length; i++) {
      ...
      if (!node.isLeaf) {
        t.step(`bit ${b}: go ${b === "0" ? "left" : "right"} (${run} so far)`,
          { snapshot: this._codecFrame({ act: 1, roles: ROLES, cur: [null, i, null], done: [null, i, null] }),
      ...
      c.out.push({ t: sym, ci });
      t.count("write");
      t.step(`${run} → '${sym}' — emitted, back to the root`,
