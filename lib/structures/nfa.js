...
// on one horizontal line. NOTE: when a single-character closure like "A*"
// produces both a char-match edge (1→2 on 'A') and an ε-edge (1→2, the
// closure "repeat" edge) on the SAME pair of states, the shared renderer
// draws them as one overlapping line/label (it doesn't offset parallel
// edges) — a known cosmetic limit of reusing the generic GraphRenderer
// rather than a bespoke automaton view; the underlying data/logic is
// unaffected (both edges are stored and both drive simulate() correctly).
  ...
  simulate(text) {
    ...
    for (let i = 0; i < text.length; i++) {
      ...
      t.step(`read '${c}' (${i + 1}/${text.length}) — states advancing on '${c}': ${show(match)}; ε-closure: ${show(pc)}`,
        { snapshot: snap(), highlight: { nodes: { active: [...pc] } } });
    }
