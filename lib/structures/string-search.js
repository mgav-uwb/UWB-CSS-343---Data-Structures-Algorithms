...
export class StringSearch {
  ...
  kmp() {
    ...
    while (i < n) {
      ...
      if (match) {
        ...
        if (j === m) {
          foundAt = i - j;
          const done = Array.from({ length: m }, (_, k) => foundAt + k);
          t.step(`full match — done, first match at index ${foundAt} (${t.counters.compare} comparisons)`, { snapshot: ksnap(), highlight: { done, pointers: { i } } });
          break;
        }
      } else if (j > 0) {
        const nj = lps[j - 1];
    ...
    if (foundAt < 0) t.step(`done — "${pattern}" does not occur in "${text}" (${t.counters.compare} comparisons)`, { snapshot: ksnap(), highlight: {} });
    const trace = t.trace(); trace.result = foundAt; return trace;
  }
}
