...
export class StringSearch {
  ...
  failTable() {
    ...
    const rowLabels = ["P", "fail"];
    const colLabels = Array.from({ length: m }, (_, i) => String(i));
    const snap = () => ({ rows: 2, cols: m, cells: [cells[0].slice(), cells[1].slice()], rowLabels, colLabels });
    const done = (upto) => { const out = []; for (let c = 0; c <= upto; c++) out.push([0, c], [1, c]); return out; };

    t.step(`build fail[] for "${p}" by matching the pattern against ITSELF — fail[j] is the longest proper prefix of P[0..j] that is also a suffix of it`,
      { snapshot: snap(), highlight: {} });
    cells[1][0] = 0; t.count("write");
    t.step(`fail[0] = 0 always: a single character has no PROPER prefix`,
      { snapshot: snap(), highlight: { active: [[1, 0]] } });
    ...
    while (j < m) {
      ...
      if (p[j] === p[k]) {
        ...
        t.step(`P[${j}]='${p[j]}' == P[${k - 1}]='${p[k - 1]}' → the overlap grows to ${k}, so fail[${j}] = ${k}`,
          { snapshot: snap(), highlight: { active: [[1, j]], from: [[0, j], [0, k - 1]], done: done(j - 1) } });
        j++;
      } else if (k > 0) {
        const was = k; k = cells[1][k - 1];
        t.count("read");
        t.step(`P[${j}]='${p[j]}' != P[${was}]='${p[was]}' → fall back through the table we already built: k = fail[${was - 1}] = ${k}. The text pointer of the SEARCH does this same move`,
          { snapshot: snap(), highlight: { active: [[0, j]], from: [[1, was - 1]], done: done(j - 1) } });
      } else {
        cells[1][j] = 0; t.count("write");
        t.step(`P[${j}]='${p[j]}' matches no prefix at all (k is already 0) → fail[${j}] = 0`,
          { snapshot: snap(), highlight: { active: [[1, j]], done: done(j - 1) } });
        j++;
      }
    }
    this.summary = `fail("${p}") = [${cells[1].join(", ")}]`;
    t.step(`done — fail = [${cells[1].join(", ")}]. On a mismatch after matching j characters, the search resets j = fail[j−1] and NEVER moves the text pointer`,
